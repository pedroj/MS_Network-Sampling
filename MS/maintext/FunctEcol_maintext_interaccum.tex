...
%[REVIEW] -------------------------------------------------------------------------
Another frequently used alternative is the Chao2 estimator, $S_{Chao2}$ \citep{Gotelli:2001uo}, which has been reported to have a limited bias for small sample sizes \citep{Colwell:1994vt,Chao:2005wp}. Instead of using counts it uses incidence frequencies ($Q_k$) among samples (number of species present in just one sample, in to samples, etc.): 
